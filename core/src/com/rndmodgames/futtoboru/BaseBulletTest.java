...
public class BaseBulletTest extends BulletTest {
	...
	public void create () {
		...
		// NOTE SHADOW NEAR AND FAR PARAMETERS
		light = shadows ? new DirectionalShadowLight(128, 128, 100f, 100f, 0.1f, 200f) : new DirectionalLight();
		light.set(0.8f, 0.8f, 0.8f, -20.5f, -21f, 1.7f);
		...
		if (width > height) {
			camera = new PerspectiveCamera(60f, 3f * width / height, 3f);
		} else {
			camera = new PerspectiveCamera(60f, 3f, 3f * height / width);
		}
		
		camera.position.set(20f, 10f, 10f);
		camera.lookAt(0, 0, 0);
		
		// camera.far is the render distance parameter for this camera/scene
		camera.far = 60f;
		camera.update();
