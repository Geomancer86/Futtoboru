 ...
 * panning. Slightly modified from the original one to understand Azerty
 * keyboards (if locale is fr_FR or fr_BE) and handle shift/ctrl to "run"
 * 
 * @author badlogic
 */
public class IntFirstPersonCameraController extends InputAdapter {
	...
	/**
	 * Try to guess if Azerty keyboard by using locale, defaults to qwerty in case
	 * of error.
	 * 
	...
	public static boolean isAzertyKeyboard() {
		...
		try {
			...
			if (inputLocale != null
					&& (inputLocale.equalsIgnoreCase("fr_FR") || inputLocale.equalsIgnoreCase("fr_BE"))) {
				return true;
			}
